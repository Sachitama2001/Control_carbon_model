...
葉・幹・根・土壌の四つの炭素プールと一つの土壌水分状態から始め、
...
| 本計画の5状態ODE | 二気象ドライバーの低次元理論を解析する | 当面の主たる理論研究系 |
...
降水を炭素過程へ結びつけ、乾燥の記憶を保持するため、一つの根域土壌水分
\(W\) を追加する。
...
したがって「四プール」は炭素区画数を指し、初期モデル全体は5状態である。
葉・幹・根の内部水貯留、水ポテンシャル、窒素、リン、個体数、サイズ分布は
第一段階には入れない。
...
示している。ただし、観測された死亡機構を一つの土壌水分閾値へ直結させない。
...
\[
f_W^G(W)=\frac{W^{n_G}}{W^{n_G}+W_{50,G}^{n_G}}
\]

とする。後に peaked-Arrhenius 型やVISIT由来関数へ交換しても、実験設計は
変更しない。
...
通常ターンオーバー率を \(\tau_i\)、枯死率を \(m_i(T,W)\) として、
...
\[
R_h(T,W,C_O)=r_{O,0}q_h(T)f_W^h(W)C_O
\]
...
## 7. Minimal water balance

最初の水収支は一つの根域バケットとする。

\[
\dot W=P(t)-E(T,W,C_L)-D(W)-Q(W).
\]

ここで \(E\) は蒸発散、\(D\) は深部排水、\(Q\) は表面流出である。
例えば、

\[
E=E_0q_E(T)f_E(W)A_L(C_L),
\qquad
A_L(C_L)=1-\exp(-k_EC_L)
\]

とする。温度は直接的な熱ストレスだけでなく、蒸発散を通じて間接的に
土壌水分を低下させる。

物理的な上限 \(0\le W\le W_{\max}\) を保つため、降水超過分を \(Q\) へ、
利用可能水を超える要求を満たさないフラックス制限へ入れる。solver後の無名の
clippingで収支を隠さない。
...
m_W(W)=
m_{W,\max}
\frac{1}{1+\exp[k_W(W-W_{50,m})]}
\]

を最小候補とする。これは水ポテンシャルやPLCを解かない経験的縮約である。
Amazon の干ばつ死亡には水理破壊の強い証拠があるため、\(W\) を機構そのもの
ではなく、死亡ハザードを予測する集約状態として扱う
([Rowland et al. 2015](https://doi.org/10.1038/nature15539))。
...
\[
m_i(T,W)=m_{i,0}+\alpha_{i,T}m_T(T)+\alpha_{i,W}m_W(W)
\]
...
\[
m_i(T,W)=m_{i,0}+\alpha_{i,T}m_T+alpha_{i,W}m_W
+\gamma_i m_Tm_W
\]
...
「温度のみ」は外部降水を固定する意味であり、温度依存蒸発散を介した \(W\) の
変化を禁止しない。この総効果に加え、機構帰属のため次の pathway-clamped
実験を行う。

- \(W\) を固定し、温度の直接的な光合成・呼吸・死亡効果だけを測る。
...
- **H4 — memory requirement:** H2またはH3が5状態モデルに現れない場合、土壌水分
  だけでは必要な記憶が不足し、NSC、損傷炭素、または植物水貯留の追加が必要である。
